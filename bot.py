...
def close(update, context):
    ...
    if len(args) != 1:
        ...
        if valid_range(index, chats[chat_id]):
            ...
            send_reply(
                update, "You have closed a claim of *$" + get_currency(c.amount) + "*"
            )
            get(update, context)
        else:
            send_reply(
                update, "Invalid claim index. Please check available claims using /list"
            )
